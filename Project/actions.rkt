...
(define (affix w)
  (begin
    ...
    (set-world-score! w (+ (world-score w) 2))
    (set-world-score! w (+(world-score w)(* 15 (lines (world-grid w)))))
...
(define (clear-lines g)
  (begin
    ...
    (cond
      [(foldr (lambda(x y)(or x y)) #f (vector->list lines)) (begin
...
(define (generate-block)
  (let ((N (- (random 100 107) 100)))
    (cond
      [(= 0 N) (block "L1" '(5 15 25 26) 1)]
      [(= 1 N) (block "L2" '(5 15 24 25) 1)]
    ...
    (let ((i x) (result #f))
      (begin
        (while(and (not(number? result)) (<= i 150))
              (cond [(number? (cdr (vector-ref g (- i 1)))) (set! result i)]) (set! i (+ 10 i)))
        result
        )
      )
    )
  (define dist (apply min (map (lambda(x) (if (number? (f x)) (- (f x) x 10) (- 140 (* 10(quotient x 10))))) l)))
  (set-world-cblock! w (block (block-type (world-cblock w)) (map (lambda(x) (+ x dist)) l) (block-state (world-cblock w))))
  w
  )
...
(define (rotate-block g b)
  (let
    ...
    (cond
      [(eq? t "line") (cond
                        [(= s 1) (cond
                                   [(foldr (lambda (x y) (or x y)) #f (map (lambda (x) (and (< x 11) (> x 0))) l)) (block t (list (cadr l) (+ 10 (cadr l)) (+ 20 (cadr l)) (+ 30 (cadr l))) 2)]
                                   [else (begin
      ...
      [(eq? t "L1") (cond
                      ...
                      [(= s 3) (let
                                 ...
                                 (if (or(foldr (lambda (x y) (or x y)) #f (map (lambda (x) (= 0 (modulo (- x 1) 10))) l)) (collision? g c))
                                     ...
                                     c))]
                      [(= s 4) (block t (list (- (caddr l) 20) (- (caddr l) 10) (caddr l) (cadddr l)) 1)])]
      [(eq? t "L2") (cond
                      [(= s 1) (let
                      ...
                      [(= s 3) (let
                                 ...
                                 (if (or(foldr (lambda (x y) (or x y)) #f (map (lambda (x) (= 0 (modulo (- x 1) 10))) l)) (collision? g c))
                                     ...
                                     c))]
                      [(= s 4) (block t (list (- (caddr l) 10) (caddr l) (+ 10 (cadr l)) (+ 10 (caddr l))) 1)])]
      [else b]
      )
    )
  )
